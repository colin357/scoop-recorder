  ...
  if (state.sent) {
    return (
      <div className="space-y-3 text-center">
        ...
        <p className="text-sm text-ink-soft">If an account exists for that address, a reset link is on its way. It&apos;s valid for one hour.</p>
        <Link className="text-sm text-merle" href="/login">Back to sign in</Link>
      </div>
    );
  }
  return (
    <form action={action} className="space-y-4">
      ...
      <p className="text-sm text-muted text-center"><Link className="text-merle" href="/login">Back to sign in</Link></p>
    </form>
  );
}
