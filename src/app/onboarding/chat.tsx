  ...
  return (
    <div className="flex-1 grid lg:grid-cols-[1fr_320px] min-h-0">
            ...
            <div className="card p-4 border-grass bg-grass-soft space-y-3">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 text-sm">
                  ...
                  <div className="text-ink-soft">{d.orgName} · {d.members.length} team member{d.members.length === 1 ? "" : "s"} · {d.projects.filter((p) => p.confirmed).length} projects</div>
                </div>
      ...
      <aside className="hidden lg:block border-l-2 border-ink bg-paper p-5 overflow-y-auto">
        ...
        <div className="mt-3">
          <div className="text-xs font-medium text-muted">Team ({d.members.length})</div>
...
function TeamWidget({ existing, self, onSubmit }: { existing: Member[]; self: { name: string; email: string }; onSubmit: (m: Member[]) => void }) {
  ...
  return (
    <div className="card p-4 space-y-3 ml-12">
      ...
      {rows.map((m, i) => (
        <div key={i} className="grid sm:grid-cols-2 gap-2 rounded-lg border border-line p-3">
