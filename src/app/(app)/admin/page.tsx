  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500">Operator view. AI cost uses AI_COST_PER_M_INPUT / AI_COST_PER_M_OUTPUT (USD per million tokens); token counts are exact.</p>
      </div>
      <div className="grid gap-4 sm:grid-cols-5">
        {[["Organizations", totals.orgs], ["Members", totals.members], ["Meetings", totals.meetings], ["Tasks", totals.tasks], ["AI cost", `$${totals.cost.toFixed(2)}`]].map(([l, v]) => (
          <div key={String(l)} className="card p-4"><div className="text-xs text-slate-500">{l}</div><div className="text-2xl font-semibold">{v}</div></div>
        ))}
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {rows.map(({ o, recent, tokIn, tokOut, hours, cost, failed, last }) => (
              <tr key={o.id}>
                <td className="p-3 font-medium">{o.name}<div className="text-xs text-slate-400">{fmtRelative(o.createdAt)}</div></td>
                ...
                <td className={`p-3 ${failed ? "text-red-600" : ""}`}>{failed}</td><td className="p-3">{hours.toFixed(1)}</td><td className="p-3">{o._count.tasks}</td>
                <td className="p-3 text-slate-500">{tokIn.toLocaleString()} / {tokOut.toLocaleString()}</td><td className="p-3">${cost.toFixed(2)}</td>
                <td className="p-3 text-slate-500">{last ? fmtRelative(last) : "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
