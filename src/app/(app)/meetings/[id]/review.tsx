...
export default function ReviewDrafts({ meetingId, drafts }: { meetingId: string; drafts: Draft[] }) {
  ...
  return (
    <div className="card p-5 border-violet-200 bg-violet-50/40 space-y-3">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-slate-600">Nobody has been notified yet. Approve what&apos;s right, open a task to edit it, or discard it.</p>
        </div>
      ...
      <ul className="divide-y divide-violet-100">
        {drafts.map((d) => (
          <li key={d.id} className="py-2 flex flex-wrap items-center gap-3 text-sm">
            <Link href={`/tasks/${d.id}`} className="flex-1 min-w-48 font-medium hover:text-indigo-600">{d.title}</Link>
            <span className="text-slate-500">{d.assignee ?? "Unassigned"} · due {d.due}</span>
            <span className="flex gap-1">
              ...
              <button className="btn-ghost !py-1 text-xs text-red-600" disabled={pending} onClick={() => start(() => discardDraftAction(d.id))}>Discard</button>
            </span>
          </li>
        ))}
      </ul>
